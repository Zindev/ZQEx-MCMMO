...
@QuestActionMark(actionID ="McMmoOperateLevel")
public class McOperateLevelAction extends QuestAction{
	...
	public void execute(Player p) {
		if(UserManager.hasPlayerDataKey(p)){
			switch (operation) {
			case "SET":
				ExperienceAPI.setLevel(p, skillType, amount);
				break;

			case "DECREASE":
				int current = ExperienceAPI.getLevel(p, skillType);
				ExperienceAPI.setLevel(p, skillType, (current-amount > -1?current-amount:0));
				break;
			}
